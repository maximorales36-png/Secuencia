#!/usr/bin/env python3
"""
SECUENCIA v3 - Detector con backends intercambiables HSV / YOLO
Deteccion dual: HSV clasico + YOLO (Ultralytics) para piezas 3D.

Backends:
  HSVDetector  -- clasificacion HSV + forma + tamaño (como v2)
  YOLODetector -- inferencia con modelo YOLO (modelos/yolo26n.pt)

Uso:
  python secuencia_detector_ipc_v3.py [indice_camara] [--mode hsv|yolo] [--calibrate] [--calibrate-colors]
"""
...
COLOR_SHAPE_MAP = {
    "red":    {"shape": "polygon",  "size_mm": 65},
    "pink":   {"shape": "polygon",  "size_mm": 55},
    "green":  {"shape": "circle",   "size_mm": 55},
    "blue":   {"shape": "hexagon",  "size_mm": 54},
    "violet": {"shape": "circle",   "size_mm": 65},
}
...
class BaseDetector:
    def detect(self, frame, corners_frac, roi_mask, M):
...
class HSVDetector(BaseDetector):
    ...
    def detect(self, frame, corners_frac, roi_mask, M):
        ...
        for color_name in self.color_ranges:
            centers = self._detect_color(frame, color_name, self.color_ranges[color_name],
                                         corners_frac=corners_frac, region_mask=roi_mask)
            if centers:
                raw[color_name] = centers

        warped = {}
        for color_name, centers in raw.items():
            warped_centers = []
            ...
            if warped_centers:
                warped[color_name] = warped_centers
        return warped

    def _detect_color(self, frame, color_name, color_range, corners_frac=None, region_mask=None):
        ...
        for r in results:
            ...
            for box, cls_id, conf in zip(r.boxes.xyxy, r.boxes.cls, r.boxes.conf):
                ...
                if label not in detections:
                    detections[label] = []
                detections[label].append((cx, cy, "yolo"))
...
def print_usage():
    ...
    print("Uso: python secuencia_detector_ipc_v3.py [indice_camara] [--mode hsv|yolo] [--calibrate] [--calibrate-colors]", file=sys.stderr)
    print(file=sys.stderr)
    print("  --mode hsv|yolo     Seleccionar backend de deteccion (default: hsv)", file=sys.stderr)
...
def main():
    ...
    for arg in sys.argv[1:]:
        if arg == "--calibrate":
            ...
            print("[v3] Filtro de forma DESACTIVADO", file=sys.stderr)
        elif arg == "--mode":
            continue
        elif arg == "hsv" or arg == "yolo":
            DETECTION_MODE = arg
            print(f"[v3] Modo de deteccion: {DETECTION_MODE.upper()}", file=sys.stderr)
        else:
            try:
    ...
    print("[SECUENCIA v3] Iniciando detector con backends intercambiables", file=sys.stderr)
    ...
    active_detector = hsv_detector if DETECTION_MODE == "hsv" else yolo_detector

    last_send_time = time.time()
    frame_count = 0
    running = True
    window_name = "SECUENCIA v3 - Detector"
    show_raw = False
    ...
    try:
        while running:
            ...
            warped_detections = active_detector.detect(frame, corners_frac, roi_mask, M)
            ...
            backend_label = active_detector.get_name()
            ...
            if key == ord('q') or key == ord('Q'):
                ...
                DETECTION_MODE = "yolo" if DETECTION_MODE == "hsv" else "hsv"
                active_detector = yolo_detector if DETECTION_MODE == "yolo" else hsv_detector
                print(f"\n[v3] Modo cambiado a: {DETECTION_MODE.upper()}", file=sys.stderr)
            elif key == ord('v') or key == ord('V'):
                show_raw = not show_raw
            elif key == ord('s') or key == ord('S'):
                SHAPE_DETECTION_ENABLED = not SHAPE_DETECTION_ENABLED
                print(f"\n[v3] Filtro de forma: {'ON' if SHAPE_DETECTION_ENABLED else 'OFF'}", file=sys.stderr)
